...
import * as dataFormat from 'pages/CRUD/Table_4/table/Table_4DataFormatters';


import actions from 'actions/table_4/table_4ListActions';
...
const { SearchBar } = Search;
  ...
  const [filters, setFilters] = React.useState([
    {label: 'newTitle', title: 'newColumn'},{label: 'tettt', title: 'tettt'},
  ]);
  ...
  const loading = useSelector((store) => store.table_4.list.loading);
  const rows = useSelector((store) => store.table_4.list.rows);
  const count = useSelector((store) => store.table_4.list.count);
  const modalOpen = useSelector((store) => store.table_4.list.modalOpen);
  const idToDelete = useSelector((store) => store.table_4.list.idToDelete);
  ...
  const actionFormatter = (cell) => {
    return (
      <div>
        {null && (
        <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/table_4/${cell}`))}>
          View
        </Button>
        )}
        <Button color="info" size="xs" onClick={() => history.push(`/admin/table_4/${cell}/edit`)}>
          Edit
        </Button>
        &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Table_4</h4>} collapse close>
<Link to="/admin/table_4/new">
  <button
