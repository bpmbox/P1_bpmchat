...
import * as dataFormat from 'pages/CRUD/Table_5/table/Table_5DataFormatters';


import actions from 'actions/table_5/table_5ListActions';
...
const { SearchBar } = Search;
  ...
  const [filters, setFilters] = React.useState([
    {label: 'newTitle', title: 'newColumn', number: 'true'},
  ]);
  ...
  const loading = useSelector((store) => store.table_5.list.loading);
  const rows = useSelector((store) => store.table_5.list.rows);
  const count = useSelector((store) => store.table_5.list.count);
  const modalOpen = useSelector((store) => store.table_5.list.modalOpen);
  const idToDelete = useSelector((store) => store.table_5.list.idToDelete);
  ...
  const actionFormatter = (cell) => {
    return (
      <div>
        {null && (
        <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/table_5/${cell}`))}>
          View
        </Button>
        )}
        <Button color="info" size="xs" onClick={() => history.push(`/admin/table_5/${cell}/edit`)}>
          Edit
        </Button>
        &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Table_5</h4>} collapse close>
<Link to="/admin/table_5/new">
  <button
