...
const { SearchBar } = Search;
  ...
  const [filters, setFilters] = React.useState([
    {label: 'Title', title: 'title'},{label: 'Author', title: 'author'},
  ]);
  ...
  const loading = useSelector((store) => store.books.list.loading);
  ...
  const actionFormatter = (cell) => {
    return (
      <div>
        {null && (
        <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/books/${cell}`))}>
          View
        </Button>
        )}
        <Button color="info" size="xs" onClick={() => history.push(`/admin/books/${cell}/edit`)}>
          Edit
        </Button>
        &nbsp;&nbsp;
...
sort: true,
  ...
  formatter: tagsDataFormat.listFormatter,
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Books</h4>} collapse close>
