...
import * as dataFormat from 'pages/CRUD/Tags/table/TagsDataFormatters';


import actions from 'actions/tags/tagsListActions';
...
const { SearchBar } = Search;
  ...
  const loading = useSelector((store) => store.tags.list.loading);
  const rows = useSelector((store) => store.tags.list.rows);
  const count = useSelector((store) => store.tags.list.count);
  const modalOpen = useSelector((store) => store.tags.list.modalOpen);
  const idToDelete = useSelector((store) => store.tags.list.idToDelete);
  ...
  const actionFormatter = (cell) => {
    return (
      <div>
        {null && (
        <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/tags/${cell}`))}>
          View
        </Button>
        )}
        <Button color="info" size="xs" onClick={() => history.push(`/admin/tags/${cell}/edit`)}>
          Edit
        </Button>
        &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Tags</h4>} collapse close>
